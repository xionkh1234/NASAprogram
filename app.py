...
def get_email_body(sandbox=True):
    ...
    data = {
        ...
        'Html-part': f'<h3>Nasa właśnie odkryło nową planete o nazwie: {planet_info[1]}!</h3><br />Odkryło ją planetarium o nazwie: {planet_info[2]} w dniu {planet_info[3]}!',
...
def send_emails():
    ...
    return [result.status_code, len(result.json()['Sent']),
            data[1]]  # 0 = kod requestu 1 = ilosc wyslanych maili 2 = ilosc maili z bazy
...
def update_planets_in_db():
    ...
    response = requests.get(
        'https://exoplanetarchive.ipac.caltech.edu/TAP/sync?query=select+distinct+pl_name,disc_facility,releasedate+from+ps+where+disc_year+>=+2021+order+by+releasedate+DESC&format=json')
        ...
        with app.app_context():  # uzycie kontekstu aplikacji wymagane do dzialania z schedulerem
            ...
            with get_db() as cursor:  # uzycie with powoduje automatyczne zamkniecie kursora po wykonianiu query
                ...
                if not integrity_check(out, planets_formated):  # brak zgodnosci db z api
                    ...
                    with get_db() as cursor:
                        res = cursor.executemany(sql_update_planets,
                                                 planets_formated)  # aktualizacja planet w bazie danych
                        if (res.rowcount > 0):
                    ...
                    with get_db() as cursor:
                        res = cursor.execute(sql_timestamp,
                                             (date_time,))  # zapisanie czasu statniej aktualizacji w bazie danych
...
scheduler.add_job(name='update_planets_in_db', id=0, func=update_planets_in_db, trigger='interval',
                  seconds=int(config['PLANETS_REFRESH_RATE']))  # utworzenie nowej pracy schedulera
...
@app.route('/debug')  # route dla latwiejszego debugowania
def debug():
    print(send_emails())
    return ('Debug')
